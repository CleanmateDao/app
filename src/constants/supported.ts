 ...
 * Includes all ISO 3166-1 alpha-2 country codes
 */
...
/**
 * Countries that support bank account integration
 * Currently only Nigeria (NG) is supported, but this can be easily extended
 */
export const BANK_SUPPORTED_COUNTRIES = ["NG"] as const;

export type BankSupportedCountryCode = (typeof BANK_SUPPORTED_COUNTRIES)[number];

/**
 * Check if a country supports bank account integration
 */
export function isBankSupported(countryCode: string): boolean {
  return BANK_SUPPORTED_COUNTRIES.includes(
    countryCode as BankSupportedCountryCode
  );
}

/**
 * Map country codes to their currency codes
 * Used for displaying currency equivalents for users in supported countries
 */
