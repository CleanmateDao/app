...
import { Sparkles, Send, ArrowUpRight, ArrowDownRight, Leaf, MapPin, Users, Gift, MoreHorizontal } from 'lucide-react';
    ...
    <div className="p-4 lg:p-6 space-y-6 lg:space-y-8 max-w-5xl mx-auto">
      ...
      >
        ...
        <div className="relative z-10">
          <div className="flex items-center justify-center gap-2 mb-4 lg:mb-6">
            ...
            <span className="text-xs lg:text-sm font-medium text-primary">Ask Temi</span>
          </div>
          ...
          <div className="flex gap-2 lg:gap-3 max-w-xl mx-auto mb-4 lg:mb-6">
            ...
            />
            <Button size="default" className="h-10 lg:h-12 px-4 lg:px-6 gap-2" onClick={() => handleAskQuestion(query)}>
      ...
      >
        {[
          { 
            label: 'Total Rewards', 
            value: `${insightsData.totalRewards} B3TR`, 
            change: '+75', 
            up: true,
            icon: Gift
          },
          { 
            label: 'Cleanups Done', 
            value: insightsData.cleanupsCompleted, 
            change: '+2', 
            up: true,
            icon: Leaf
          },
          { 
            label: 'Nearby Events', 
            value: insightsData.activeCleanupsNearby, 
            change: 'Active', 
            up: null,
            icon: MapPin
          },
          { 
            label: 'People Helped', 
            value: insightsData.participantsHelped, 
            change: '+23', 
            up: true,
            icon: Users
          },
          ...
          return (
            <motion.div key={i} variants={fadeIn}>
              <Card className="border-border/50 bg-card/50 relative overflow-hidden h-full">
                ...
                <div className={`absolute -bottom-12 -right-12 w-24 h-24 rounded-full border-[3px] ${ringColor}/10`} />
                <div className={`absolute -bottom-8 -right-8 w-16 h-16 rounded-full border-[3px] ${ringColor}/15`} />
                <div className={`absolute -bottom-4 -right-4 w-8 h-8 rounded-full border-[3px] ${ringColor}/20`} />
                ...
                <CardContent className="p-3 sm:p-4 lg:p-5 relative z-10 h-full flex flex-col">
                  ...
                  <div className="flex items-end justify-between mt-auto">
                    <span className="text-lg sm:text-xl lg:text-2xl font-semibold">{metric.value}</span>
                    {metric.up !== null && (
                      <span className={`flex items-center text-[10px] sm:text-xs ${metric.up ? 'text-status-approved' : 'text-status-rejected'}`}>
                        {metric.up ? <ArrowUpRight className="h-3 w-3" /> : <ArrowDownRight className="h-3 w-3" />}
                        {metric.change}
                      </span>
                    )}
                    {metric.up === null && (
                      <span className="text-[10px] sm:text-xs text-muted-foreground">{metric.change}</span>
                    )}
                  </div>
                </CardContent>
              </Card>
        ...
        <Card className="border-border/50 bg-card/50 overflow-hidden">
          <CardContent className="p-4 lg:p-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4 lg:mb-6">
              <div>
                <h3 className="font-medium text-sm lg:text-base">Activity Trend</h3>
                <p className="text-xs lg:text-sm text-muted-foreground">Cleanups & Rewards over time</p>
              </div>
            ...
            <div className="h-40 lg:h-48">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={insightsData.monthlyData}>
                  <defs>
        ...
        <Card className="border-border/50 bg-card/50 relative overflow-hidden">
          ...
          <CardContent className="p-4 lg:p-6 relative z-10">
            <div className="flex items-center justify-between mb-4 lg:mb-6">
              <h3 className="font-medium text-sm lg:text-base">Cleanups by Category</h3>
              <span className="text-xs text-muted-foreground">Distribution</span>
            </div>
            
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 lg:gap-4">
              {insightsData.categoryData.map((item, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div 
                      ...
                      style={{ 
                        background: `radial-gradient(circle at center, ${item.fill.replace(')', ' / 0.1)')}, transparent 70%)` 
                      }}
                    />
                    ...
                    <div className="relative z-10">
                      ...
                      <div className="flex items-center justify-between mb-3">
                        <div 
                          ...
                          style={{ backgroundColor: item.fill.replace(')', ' / 0.15)') }}
                        >
